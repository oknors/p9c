...
${``}`;
  ...
  function init(settings = default_settings) {
    ...
    options = {
      ...
      dev: false,
      entry: {
        file: assets + "/_app/start-4a486205.js",
        css: [],
        js: [assets + "/_app/start-4a486205.js", assets + "/_app/chunks/vendor-633de1bb.js"]
      },
      fetched: void 0,
      floc: false,
...
<div id="${"layout"}" class="${"relative flex flex-col items-center justify-between min-h-screen antialiased font-text text-text-primary "}">${validate_component(Bg, "Bg").$$render($$result, {}, {}, {})}
  ...
  var Where = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    return `<div class="${"flex flex-col text-center parallelcoin"}">rrrrr
        ${validate_component(HeadTitle, "HeadTitle").$$render($$result, {}, {}, {
      default: () => `Where is ParallelCoin???`
    })}
        <div class="${"my-24 worp"}">sssss
            <amp-list layout="${"responsive"}" width="${"600"}" height="${"300"}" src="${"https://api.parallelcoin.info/subs"}" reset-on-refresh single-item binding="${"no"}">${escape(`{{ . }}`)}
            <template type="${"amp-mustache"}"><a class="${"flex flex-row justify-center my-4 text-2xl no-underline text-text-primary"}"${add_attribute("href", `'https://'{{ slug }}'.parallelcoin.info'`, 0)}><div class="${"character"}">${escape(`{{ title }}`)}</div>
            <div class="${"flex-grow hidden mx-1 mb-2 border-b-2 border-dotted md:flex"}"></div>
